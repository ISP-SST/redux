...
size_t Constraints::size( void ) const {
    size_t sz = ( 4 + 2 * nGroups + n_omap + n_imap ) * sizeof( int32_t ); // ng,nv,nh,n_omap,n_imap,omap_n,imap_n,nrv
    for( int g=0; g < nGroups; ++g ) sz += nVerticals[g] * nHorizontals[g] * sizeof( double );
    for( int o=0; o < n_omap; ++o ) sz += omap_n[o] * sizeof( int32_t );
    for( int i=0; i < n_imap; ++i ) sz += imap_n[i] * sizeof( int32_t );
    sz += 5 * nrv * sizeof( int32_t ); // rv
    return sz;
}
...
char* Constraints::pack( char* ptr ) const {
    ...
    for( size_t g = 0; g < nGroups; ++g ) ptr = pack( ptr, *( groups[g].get() ), nVerticals[g] * nHorizontals[g] );
    ptr = pack( ptr, n_omap );
    ptr = pack( ptr, omap_n );
    for( size_t o = 0; o < n_omap; ++o ) ptr = pack( ptr, o_map[o] );
    ptr = pack( ptr, n_imap );
    ptr = pack( ptr, imap_n );
    for( size_t i = 0; i < n_imap; ++i ) ptr = pack( ptr, i_map[i] );
    ptr = pack( ptr, nrv );
    int32_t gvoi[4];
    for( int k = 0; k < nrv; ++k ) {
        memset(gvoi,0,4*sizeof(int32_t));
        for( int ig=0; ig < nGroups; ++ig ) // search for nullspace index
            for( int iv=0; iv < nVerticals[ig]; ++iv )
        ...
        for( int io=0; io < n_omap; ++io ) // search for object map index
            if( rv[k].o_map == o_map[io].data() ) {
                gvoi[2] = io;
                io = n_omap;
            }
        for( int ii=0; ii < n_imap; ++ii ) // search for image map index
