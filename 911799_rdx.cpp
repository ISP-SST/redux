...
#include "redux/polarimetry/mueller.hpp"
#include "redux/polarimetry/rotation.hpp"
#include "redux/polarimetry/retarder.hpp"
#include "redux/polarimetry/polarizer.hpp"
...
using namespace redux::polarimetry;
...
IDL_VPTR make_pupil(int argc, IDL_VPTR* argv, char* argk) {
    ...
    if (argc < 2) {
        cout << "pupil: needs 2 arguments: nPixels & pupil-radius (in pixels). " << endl;
        return IDL_GettmpInt (-1);
    }
...
IDL_VPTR karhunen_loeve(int argc, IDL_VPTR* argv, char* argk) {
    
    if (argc < 3) {
        cout << "karhunen_loeve: needs 3 arguments: mode-number(s), nPixels & pupil-radius (in pixels). " << endl;
        return IDL_GettmpInt (-1);
    }
    ...
    if ( (index->flags & IDL_V_ARR) ) {
        if( index->type == IDL_TYP_BYTE ) {
            ...
            cout << "karhunen_loeve: mode-numbers must be of type BYTE, INT or LONG." << endl;
            return IDL_GettmpInt (-1);
        }
    } else {
...
IDL_VPTR cpolcal_2d(int argc, IDL_VPTR* argv, char* argk) {
    
    if (argc < 4) {
        cout << "cpolcal_2d: needs 4 arguments. " << endl;
        return IDL_GettmpInt (-1);
    }
    ...
    if (kw.help) {
        cout << "cpolcal_2d: no help available yet..." << endl;
        return IDL_GettmpInt (-1);
    }
    ...
    IDL_ENSURE_SIMPLE (qw);
    IDL_ENSURE_ARRAY (qw);

    IDL_ENSURE_SIMPLE (lp);
    IDL_ENSURE_ARRAY (lp);
    
    IDL_ENSURE_SIMPLE (init);
    IDL_ENSURE_ARRAY (init);

    int nDims = data->value.arr->n_dim;
cout << "nd=" << nDims << printArray(data->value.arr->dim,nDims,"  dims") << endl;
    if ( nDims != 5 ) {
        cout << "cpolcal_2d: data must be a 5-dimensinal cube with dims=[nLC,nQW,nLP,xPixels,yPixels]" << endl;
        return IDL_GettmpInt (-1);
    }
    if ( lp->value.arr->n_dim != 1 || qw->value.arr->n_dim != 1 || init->value.arr->n_dim != 1 ) {
        cout << "cpolcal_2d: LP/QW/init must be 1-dimensional arrays." << endl;
        return IDL_GettmpInt (-1);
    }
    
    int nLC = data->value.arr->dim[0];
    int nQW = data->value.arr->dim[1];
    int nLP = data->value.arr->dim[2];
    if ( nLC > 4 ) {
        cout << "cpolcal_2d: nLC must be <= 4." << endl;
        return IDL_GettmpInt (-1);
    }
    if ( nQW != qw->value.arr->dim[0]) {
        cout << "cpolcal_2d: data size does not match QW size." << endl;
        return IDL_GettmpInt (-1);
    }
    if ( nLP != lp->value.arr->dim[0]) {
        cout << "cpolcal_2d: data size does not match LP size." << endl;
        return IDL_GettmpInt (-1);
    }
    if ( init->value.arr->dim[0] != 16 ) {
        cout << "cpolcal_2d: initial guess must contain 16 elements." << endl;
        return IDL_GettmpInt (-1);
    }
    
    Mueller m;
    cout << "Mueller: " << m << endl;
    MRotation rot(M_PI/4);
    cout << "Rot(" << (M_PI/4) << "): " << rot << endl;
    MPolarizer pol(1E-3);
    cout << "Pol: " << pol << endl;
    MRetarder ret(M_PI/4,0);
    cout << "Ret(" << (M_PI/4) << "): " << ret << endl;
    cout << "RotRetRot(" << (M_PI/4) << "): " << (rot.inverse()*ret*rot) << endl;
    if (nLC != 1 || nDims != lp->value.arr->n_dim || nDims != qw->value.arr->n_dim) {
        cout << "cpolcal_2d: input must be 1-dimensional arrays." << endl;
        return IDL_GettmpInt (-1);
    }
    
    if (data->type < 4 || qw->type < 4 || lp->type < 4 ||
        data->type > 5 || qw->type > 5 || lp->type > 5) {
        cout << "cpolcal_2d: ERROR, input data must be double or float!" << endl;
        return IDL_GettmpInt (-1);
    }

    int n =  data->value.s.arr->dim[0];
    int np = lp->value.s.arr->dim[0];
    double *res = new double[np];
...
extern "C" {
    ...
    int IDL_Load (void) {
        ...
        static IDL_SYSFUN_DEF2 function_addr[] = {
            ...
            { { (IDL_VPTR (*) ()) make_pupil}, (char*) "CPUPIL", 2, 2, IDL_SYSFUN_DEF_F_KEYWORDS, 0 },
            { { (IDL_VPTR (*) ()) karhunen_loeve}, (char*) "KARHUNEN_LOEVE", 3, 3, IDL_SYSFUN_DEF_F_KEYWORDS, 0 },
            ...
            { { (IDL_VPTR (*) ()) cpolcal_2d}, (char*) "CPOLCAL_2D", 4, 4, IDL_SYSFUN_DEF_F_KEYWORDS, 0 }

        };

        static IDL_SYSFUN_DEF2 procedure_addr[] = {
            { { (IDL_SYSRTN_GENERIC) redux::printStruct}, (char*) "STRUCT_INFO", 0, 1, IDL_SYSFUN_DEF_F_KEYWORDS, 0 },
            { { (IDL_SYSRTN_GENERIC) clear_modecache}, (char*) "CLEAR_MODECACHE", 0, 0, IDL_SYSFUN_DEF_F_KEYWORDS, 0 },
