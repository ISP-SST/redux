...
JobTab::JobTab( Job::JobSet& j, QWidget* par, const char* name ) : QWidget( par ), jobs(j), modified( false ) {
...
void JobTab::init( void ) {


}

void JobTab::loadJob( int active ) {
...
void JobTab::contextMenuEvent( QContextMenuEvent* event ) {
...
void JobTab::createLayout( void ) {
    ...
    globalStatusLabel->setText( tr( "OGGLINE" ) );

    globalLogLabel = new QLabel( globalFrame );
    globalLogLabel->setText( tr( "Job Log:" ) );
    ...
    globalLayout->addWidget( globalStatusLabel, 0, 3, 1, 1 );
    globalLayout->addWidget( globalLogLabel, 1, 0, 1, 1 );
    globalLayout->addWidget( globalVerbLabel, 1, 2, 1, 1 );
    globalLayout->addWidget( globalVerbCombo, 1, 3, 1, 1 );
...
void JobTab::createActions( void ) {
...
void JobTab::loadValues( void ) {

    globalStatusLabel->setText( tr( "ONLINE" ) );
    globalStatusLabel->setForegroundRole( QPalette::Highlight );

    globalLognameLabel->setText( "logname..." );
    //globalLognameLabel->setForegroundRole ( QPalette::Highlight );

    globalVerbCombo->setCurrentIndex( 0 ); //myNet->sysLog.cfg.verbosity );
